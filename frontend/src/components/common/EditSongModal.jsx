...
export function EditSongModal({ song, onClose, onSaved }) {
    ...
    const [coverPreview, setCoverPreview] = useState(normalizeSong(song).coverUrl);
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-surface-dark border border-border-dark rounded-2xl w-full max-w-lg p-6 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex items-start gap-4 mb-5">
                    ...
                    >
                        {coverPreview ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center">
                                <span className="material-symbols-outlined text-3xl text-primary/40">music_note</span>
                            </div>
                        )}
                    ...
                    <div className="flex-1 space-y-3">
                        ...
                        <div>
                            <label className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Visibility</label>
                            <select
                            ...
                            >
                                <option value="public" className="bg-charcoal text-white">Public</option>
                                <option value="draft" className="bg-charcoal text-white">Draft</option>
                                <option value="private" className="bg-charcoal text-white">Private</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <div>
                        <label className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Genre</label>
                        <select
                            value={form.genre}
                        ...
                        >
                            ...
                            {ALL_GENRES.map((g) => <option key={g} value={g} className="bg-charcoal text-white">{g}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Mood</label>
                        <select
                            value={form.mood}
                            onChange={(e) => setForm((f) => ({ ...f, mood: e.target.value }))}
                            className="w-full bg-charcoal border border-border-dark rounded-lg px-3 py-2.5 text-white text-sm cursor-pointer focus:ring-2 focus:ring-primary outline-none"
                        >
                            <option value="" className="bg-charcoal text-white">Select mood...</option>
                            {ALL_MOODS.map((m) => <option key={m} value={m} className="bg-charcoal text-white">{m}</option>)}
                        </select>
                    </div>
                    <div>
                ...
                <div className="flex gap-3 mt-6">
                    <button onClick={onClose} className="flex-1 border border-border-dark text-slate-300 py-2.5 rounded-full hover:bg-border-dark transition-colors cursor-pointer text-sm">
                        Cancel
                    </button>
                    <button
                        onClick={save}
